...
<head>
    ...
    <meta property="og:url" content="https://tanaismenendez.es/{slug}">
    ...
    <meta property="twitter:domain" content="tanaismenendez.es">
    ...
    <script>
        window.location.href = "https://tanaismenendez.es/portfolio#{slug}";
    </script>
    
    <meta http-equiv="refresh" content="0;url=https://tanaismenendez.es/portfolio#{slug}">
</head>
<body>
    <p>Abriendo proyecto... <a href="https://tanaismenendez.es/portfolio#{slug}">Click aquí si no redirige.</a></p>
</body>
</html>"""
...
def to_title_case(text):
    ...
    # Primero todo a minúscula, luego procesamos palabra por palabra
    words = text.split()
    new_words = [replace(re.search(r'\w+', words[0], re.UNICODE))] # Primera palabra siempre Capitalizada
    
    for i in range(1, len(words)):
        # Si la palabra anterior termina en : o | forzamos mayúscula
        prev_char = words[i-1][-1] if words[i-1] else ''
        if prev_char in [':', '|', '.', '-']:
             new_words.append(words[i].capitalize()) # Simplificación, idealmente usar lógica replace forzando upper
        else:
             new_words.append(replace(re.search(r'\w+|.', words[i], re.UNICODE))) # Simplificación

    # Nota: Para simplificar y asegurar robustez, usamos una lógica mixta simple:
    # Si quieres perfección absoluta en Title Case, usa la librería 'titlecase' de python,
    # pero este aproximación manual funciona bien para tus títulos.
    processed = []
...
def update_index(videos):
    ...
    for v in top_videos:
        ...
        # Estructura específica del index (con data-aos y figure)
        html_out += f"""
...
def update_portfolio(videos):
    ...
    for v in videos:
        title_upper = html.escape(v['title'].upper())
        # Convertir saltos de línea en párrafos HTML para el div oculto
        ...
        <div class="video filter {v['category']}">
            <a href="#{v['slug']}" data-youtube-id="{v['youtube_id']}">
                ...
                <img src="https://tanaismenendez.es/img/covers/{v['slug']}.jpg" loading="lazy" alt="{title_upper}">
            </a>
